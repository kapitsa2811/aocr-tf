...
def _extract_argmax_and_embed(embedding, output_projection=None,
    ...
    def loop_function(prev, _):
        ...
        # prev_symbol = tf.Print(prev_symbol, [tf.shape(prev_symbol), prev_symbol],"PS:", summarize=5)
...
def attention_decoder(decoder_inputs, initial_state, attention_states, cell,
    ...
    with tf.variable_scope(scope or "attention_decoder"):
        ...
        for i, inp in enumerate(decoder_inputs):
            ...
            inp = tfprint(inp, f"{i}$INP_B:")
            if loop_function is not None and prev is not None:
                with tf.variable_scope("loop_function", reuse=True):
                    inp = loop_function(prev, i)
            inp = tfprint(inp,f"{i}$INP_A:")
            # Merge input and previous attentions into one vector of the right size.
            # input_size = inp.get_shape().with_rank(2)[1]
            # todo: use input_size
            input_size = attn_num_hidden
            attns[0] = tfprint(attns[0], f"{i}$ATTNS_B:")
            lin_inp = [inp] + attns
            # print(lin_inp)
            x = linear(lin_inp, input_size, True)
            x = tfprint(x, f"{i}$X:")



            # Run the RNN.
            state = tfprint(state,f"{i}$AF_ST:") #1024, 512
...
def embedding_attention_decoder(decoder_inputs, initial_state, attention_states,
    ...
    with tf.variable_scope(scope or "embedding_attention_decoder"):
        with tf.device("/cpu:0"):
            ...
            # print(embedding, num_symbols, embedding_size)
        loop_function = _extract_argmax_and_embed(
            embedding, output_projection,
            update_embedding_for_previous) if feed_previous else None
        emb_inp = []
        for i, inp in enumerate(decoder_inputs):
            # inp = tf.Print(inp, [tf.shape(inp),inp], f"{i}DII:", summarize=10)
            embi = tf.nn.embedding_lookup(embedding, inp)
            # embi = tf.Print(embi, [tf.shape(embi),embi], f"{i}EMB:", summarize=10)
            emb_inp.append(embi)

        # print("EMB_LEN",len(emb_inp))
        # emb_inp[0] = tf.Print(emb_inp[0],[tf.shape(emb_inp[0]),emb_inp[0], tf.shape(decoder_inputs[0]), decoder_inputs[0] ],"EMB:",summarize=10)
